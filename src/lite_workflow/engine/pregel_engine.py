...
from ..definitions.graph import Graph, NodeId
...
class PregelEngine(ExecutionEngine):
    ...
    async def _execute_superstep_async(
        self,
        messages: Dict[NodeId, List[Dict[str, Any]]],
        superstep: int
        ...
        for node_id, result in zip(active_nodes, results):
            if isinstance(result, Exception):
                ...
                try:
                    ...
                    raise RuntimeError(f"Node {node_id} failed permanently: {e}")
    ...
    async def _execute_node_async(
        ...
        try:
            ...
            self.event_bus.emit(NodeExecutionEvent(
                node_id=node_id,
                duration=duration,
                outputs=outputs
            ))
    ...
    def get_progress(self) -> Dict[str, Any]:
        ...
        executed_nodes = len(self._execution_stats["node_execution_times"])
        
        return {
            "total_nodes": total_nodes,
            "executed_nodes": executed_nodes,
            "progress_percent": (executed_nodes / total_nodes) * 100 if total_nodes > 0 else 0,
            "messages_sent": self._execution_stats["messages_sent"],
            "current_superstep": self._execution_stats["total_supersteps"]
        }